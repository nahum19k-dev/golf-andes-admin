...
def leer_pagos_mes(mes: str, anio: int):
    ...
    # Debug: mostrar qué hoja estamos buscando (puedes eliminar después)
    # st.write(f"Buscando hoja: {nombre_hoja}")
    spreadsheet = get_spreadsheet()
    try:
        worksheet = spreadsheet.worksheet(nombre_hoja)
    except gspread.exceptions.WorksheetNotFound:
    ...
    # Debug: mostrar columnas encontradas (puedes eliminar después)
    ...
    # st.write(f"fecha: {col_fecha}, torre: {col_torre}, dpto: {col_dpto}, ingresos: {col_ing}, oper: {col_oper}, mant: {col_mant}, amort: {col_amort}, med: {col_med}")

    if not (col_fecha and col_torre and col_dpto and col_ing):
        # st.warning("No se encontraron columnas esenciales (fecha, torre, dpto, ingresos)")
        return pd.DataFrame()
